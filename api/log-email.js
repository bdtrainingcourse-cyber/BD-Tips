...
async function handleScheduleBulkAlumniLaunching(req, res) {
  ...
  // If testing or dryRun and candidate list from Google Sheet is empty (e.g. preview run), provide safe preview candidates
  if ((!candidateList || candidateList.length === 0) && (isDryRun || isTestMode)) {
    candidateList = [
      { name: 'Peter Võ (Test Preview)', email: 'vptanaia@gmail.com', nickname: 'Chiến Thần BD', vipCode: 'BD-PREVIEW1' },
      { name: 'Học Viên Test 2', email: 'bdtrainingcourse@gmail.com', nickname: 'Chúa Tể Chốt Deal', vipCode: 'BD-PREVIEW2' },
      { name: 'Học Viên Test 3', email: 'bdmastery.ai@petervo.vn', nickname: 'Sát Thủ Săn Lead', vipCode: 'BD-PREVIEW3' }
    ];
  }
